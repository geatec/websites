...
# To use parameters, derive a dynamic page class from class Page.
	...
	def get (self):
		...
		clairvoyant = len (set ([userRole ['name'] for userRole in self.userRoles]) & set ((self.app.superUserRoleName, self.app.clairUserRoleName))) > 0
		
		for arg in self.args:
			if arg.startswith ('_') and not clairvoyant:
